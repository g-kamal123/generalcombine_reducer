...
const Icecream = (props) => {
  ...
  const btnHandler =()=>{
    ...
    if(num>props.IceCreamReducer.icecream){
      setError(true)
      return
    }
    props.buy(buy_Icecream,num)
    setError(false)
    setOrderStatus(true)
  }
  return (
    <Card sectioned>
      <Heading>Number of available Icecream: {props.IceCreamReducer.icecream}</Heading>
      <TextField
        placeholder="enter number of icereams to buy"
        type="number"
        value={num}
        onChange={inpHandler}
      />
...
export default connect(mapToState,mapToDispatch)(Icecream);
